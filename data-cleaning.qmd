...
setwd("~/Documents/EDS213/week2/bren-meds213-data-cleaning")
...
Check NAs
```{r}
# percentage of NAs in each of the columns
snowsurvey_csv |> 
  miss_var_summary()
```
...
```{r}
write_csv(snowsurvey_fixed, here("data", "processed", "snow_cover.csv"))
```
...
```{r}
# Import the species
species_csv <- read_csv(here("data", "raw", "ASDN_Daily_species.csv"))
...
```{r}
write_csv(species_presence, file.path(here("data", "processed", "species_presence.csv")))
```
